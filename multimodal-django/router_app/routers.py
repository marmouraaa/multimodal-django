    ...
    def process(self, parsed_content, question):
        ...
        try:
            if modality == "document":
                ...
                logger.info(f"[ROUTER] Document - text length: {len(text_content)}")
                answer, model_conf = self.text_model.answer(text_content, question)

            elif modality == "image":
                # FIX: essayer toutes les clés possibles pour l'image
                image_content = (
                    parsed_content.get("image_path")
                    or parsed_content.get("image")
                    or parsed_content.get("file_path")
                )
                logger.info(f"[ROUTER] Image - image_content: {image_content}")
                if not image_content:
                    return RoutingResult(
                        answer="Aucune image fournie ou fichier introuvable.",
                        confidence=0.0, modality=modality,
                        fallback_used=True, fallback_method="no_image",
                        method=method, mime=mime
                    )
                answer, model_conf = self.vision_model.analyze(image_content, question)

            elif modality == "structured":
                # FIX: utiliser "data" qui contient maintenant le texte extrait,
                # ou fallback sur "text"
                data_content = parsed_content.get("data") or parsed_content.get("text", "")
                logger.info(f"[ROUTER] Structured - data type: {type(data_content)}, length: {len(str(data_content))}")
